...
class UsersSettings extends Users
{
    ...
    public function userSwitchActiveCheck(): void
    {
    ...
    public function disableUserInactivityCheck(): void
    {
        ...
        <input type='checkbox' name='moj_component_settings[user_active_disable]'
               ...
               class="moj-component-input-checkbox">
        <p>User activity checks are useful while testing and on production servers.
           You can switch off the checking process here.</p>
        <?php
    }

    public function testUserInactivityCheck(): void
    {
        $options = get_option('moj_component_settings');
        ?>
        <input type='checkbox' name='moj_component_settings[user_inactive_test]'
               ...
               class="moj-component-input-checkbox">
        <p>Check this option if you would like to test the Inactivity Check operation.
           An email with dummy data will be sent to the email address above.<br>
           Please reduce the schedule time for testing and make sure to reset when satisfied.</p>
        <?php
    }

    public function userInactiveSchedule(): void
    {
        $options   = get_option('moj_component_settings');
        ...
        foreach ($items as $item) {
            ...
            echo '<option value="' . esc_attr($item['name']) . '" ' . $selected . '>'
                . esc_html($item['display']) . '</option>';
        }
        echo '</select>';
    }
    ...
    {
        echo __(
            '<p>The Users section of the Components plugin seeks to monitor user activity
             and report unusual occurrences or aged logins.</p>',
            'wp-moj-components'
        );
    }
}
