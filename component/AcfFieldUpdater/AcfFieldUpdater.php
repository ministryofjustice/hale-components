...
/**
 * Displays a form in the WordPress admin to update ACF meta keys or meta values
    ...
    public function renderTool(): void
    {
        ...
        if (isset($_POST['update_acf_field'])) {
            ...
            if ($updateType === 'key') {
                ...
                $sql = $wpdb->prepare(
                    "UPDATE {$wpdb->postmeta} SET meta_value = %s WHERE meta_value = %s AND meta_key = %s",
                    $newValue,
                    $oldValue,
                    $metaKey
                );
            ...
            if ($updatedRows > 0) {
                echo '<div class="updated"><p>ACF field updated successfully. Rows affected: ' . (int) $updatedRows . '</p></div>';
            } else {
                echo '<div class="error"><p>No rows updated. The old meta key or value may not exist or was already updated.</p></div>';
            }
        }
        ?>
