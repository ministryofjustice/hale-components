...
class TaxonomyUpdater
{
    public function renderTool(): void
    {
        ...
        if (isset($_POST['update_taxonomy'])) {
            if (
                !isset($_POST['taxonomy_updater_nonce']) ||
                !wp_verify_nonce($_POST['taxonomy_updater_nonce'], 'taxonomy_updater_action')
            ) {
            ...
            $oldSlug = sanitize_text_field($_POST['old_slug']);
            ...
            if ($updatedRows > 0) {
                echo '<div class="updated"><p>Taxonomy updated successfully. Rows affected: ' . (int) $updatedRows . '</p></div>';
            } else {
                echo '<div class="error"><p>No rows updated. The old slug may not exist or was already updated.</p></div>';
            }
        }
        ?>
        ...
        <div class="wrap">
            ...
            <p>Run a database update on the current site that changes a taxonomy entry in the database to a new one.
               ...
               <br>Once run, all content related to the old taxonomy will be associated with the new taxonomy name.</p>
